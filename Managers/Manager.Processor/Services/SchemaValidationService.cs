...
public class SchemaValidationService : ISchemaValidationService
{
    ...
    public async Task ValidateSchemasExist(Guid inputSchemaId, Guid outputSchemaId)
    {
        ...
        {
            _logger.LogDebug("Schema validation is disabled. Skipping validation for InputSchemaId: {InputSchemaId}, OutputSchemaId: {OutputSchemaId}", 
        ...
        if (inputSchemaId == Guid.Empty)
        {
            ...
            _logger.LogWarning("Schema validation failed: {Message}", message);
        ...
        _logger.LogDebug("Validating schemas exist. InputSchemaId: {InputSchemaId}, OutputSchemaId: {OutputSchemaId}", 
            inputSchemaId, outputSchemaId);
        ...
        try
        {
            ...
            _logger.LogDebug("Schema validation passed. InputSchemaId: {InputSchemaId}, OutputSchemaId: {OutputSchemaId}", 
                inputSchemaId, outputSchemaId);
        }
        ...
        {
            _logger.LogError(ex, "Schema validation failed due to unexpected error. InputSchemaId: {InputSchemaId}, OutputSchemaId: {OutputSchemaId}", 
                inputSchemaId, outputSchemaId);
