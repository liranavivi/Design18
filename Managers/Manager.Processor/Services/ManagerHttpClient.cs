...
public class ManagerHttpClient : IManagerHttpClient
{
        ...
        ILogger<ManagerHttpClient> logger)
    {
        ...
        _logger = logger;
        _stepManagerBaseUrl = configuration["ManagerUrls:StepManager"]
            ?? throw new InvalidOperationException("StepManager URL not configured");
        _schemaManagerBaseUrl = configuration["ManagerUrls:Schema"] ?? "http://localhost:5000";
        ...
        var retryPolicy = HttpPolicyExtensions
            ...
            .WaitAndRetryAsync(
                ...
                {
                    _logger.LogWarning("Retry {RetryCount} for {Operation} after {Delay}ms. Reason: {Reason}",
        ...
        var circuitBreakerPolicy = HttpPolicyExtensions
            ...
            .CircuitBreakerAsync(
                ...
                {
                    _logger.LogError("Circuit breaker opened for {Duration}s. Reason: {Reason}",
    ...
    private async Task<bool> ExecuteProcessorCheck(string url, string operationName, Guid processorId)
    {
        try
        {
            _logger.LogDebug("Starting {Operation} for ProcessorId: {ProcessorId}, URL: {Url}", 
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogDebug("Completed {Operation} for ProcessorId: {ProcessorId}. HasReferences: {HasReferences}", 
            ...
            else
            {
                ...
                _logger.LogError("HTTP error in {Operation} for ProcessorId: {ProcessorId}. Status: {StatusCode}, Content: {Content}",
        ...
        {
            _logger.LogError(ex, "Circuit breaker is open for {Operation}. ProcessorId: {ProcessorId}",
        ...
        {
            _logger.LogError(ex, "HTTP request failed for {Operation}. ProcessorId: {ProcessorId}",
                operationName, processorId);
        ...
        {
            _logger.LogError(ex, "Timeout in {Operation} for ProcessorId: {ProcessorId}",
                operationName, processorId);
            throw new InvalidOperationException($"Processor reference validation timed out. Operation rejected for safety.", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in {Operation} for ProcessorId: {ProcessorId}",
                operationName, processorId);
    ...
    private async Task<bool> ExecuteSchemaCheck(string url, string operationName, Guid schemaId)
    {
        try
        {
            _logger.LogDebug("Starting {Operation} for SchemaId: {SchemaId}, URL: {Url}",
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogDebug("Completed {Operation} for SchemaId: {SchemaId}. Exists: {Exists}",
            ...
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                _logger.LogError("HTTP error in {Operation} for SchemaId: {SchemaId}. Status: {StatusCode}, Content: {Content}",
        ...
        {
            _logger.LogError(ex, "Circuit breaker is open for {Operation}. SchemaId: {SchemaId}",
