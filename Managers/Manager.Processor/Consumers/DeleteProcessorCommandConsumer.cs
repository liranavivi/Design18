...
public class DeleteProcessorCommandConsumer : IConsumer<DeleteProcessorCommand>
{
    ...
    public async Task Consume(ConsumeContext<DeleteProcessorCommand> context)
    {
        ...
        _logger.LogInformation("Processing DeleteProcessorCommand. Id: {Id}, RequestedBy: {RequestedBy}",
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Processor entity not found for deletion. Id: {Id}", command.Id);
            ...
            if (success)
            {
                ...
                stopwatch.Stop();
                _logger.LogInformation("Successfully processed DeleteProcessorCommand. Id: {Id}, Duration: {Duration}ms",
            ...
            {
                stopwatch.Stop();
                _logger.LogWarning("Failed to delete Processor entity. Id: {Id}, Duration: {Duration}ms",
                    command.Id, stopwatch.ElapsedMilliseconds);
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Error processing DeleteProcessorCommand. Id: {Id}, Duration: {Duration}ms",
                command.Id, stopwatch.ElapsedMilliseconds);
