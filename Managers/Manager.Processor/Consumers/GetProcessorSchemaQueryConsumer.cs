...
public class GetProcessorSchemaQueryConsumer : IConsumer<GetProcessorSchemaQuery>
{
    ...
    public async Task Consume(ConsumeContext<GetProcessorSchemaQuery> context)
    {
        ...
        _logger.LogInformation("Processing GetProcessorSchemaQuery. ProcessorId: {ProcessorId}, RequestedBy: {RequestedBy}",
        ...
        try
        {
            ...
            if (entity != null)
            {
                _logger.LogInformation("Successfully processed GetProcessorSchemaQuery. Found Processor Id: {Id}, InputSchemaId: {InputSchemaId}, OutputSchemaId: {OutputSchemaId}, Duration: {Duration}ms",
            ...
            {
                _logger.LogWarning("Processor entity not found. ProcessorId: {ProcessorId}, Duration: {Duration}ms",
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Error processing GetProcessorSchemaQuery. ProcessorId: {ProcessorId}, Duration: {Duration}ms",
                query.ProcessorId, stopwatch.ElapsedMilliseconds);
