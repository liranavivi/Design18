...
public class ManagerHttpClient : IManagerHttpClient
{
    ...
    public async Task<bool> CheckEntityExists(Guid entityId)
    {
        ...
        _logger.LogDebug("Basic entity existence check for EntityId: {EntityId}", entityId);
        ...
        {
            _logger.LogWarning("Entity existence check failed - empty GUID. EntityId: {EntityId}", entityId);
            return false;
        }
        ...
        _logger.LogDebug("Entity existence check passed (basic validation). EntityId: {EntityId}", entityId);
        return true;
    }
