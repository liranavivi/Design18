...
public class DeleteAssignmentCommandConsumer : IConsumer<DeleteAssignmentCommand>
{
    ...
    public async Task Consume(ConsumeContext<DeleteAssignmentCommand> context)
    {
        ...
        _logger.LogInformation("Processing DeleteAssignmentCommand. Id: {Id}, RequestedBy: {RequestedBy}",
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Assignment entity not found for deletion. Id: {Id}", command.Id);
            ...
            if (success)
            {
                ...
                stopwatch.Stop();
                _logger.LogInformation("Successfully processed DeleteAssignmentCommand. Id: {Id}, Duration: {Duration}ms",
            ...
            {
                stopwatch.Stop();
                _logger.LogWarning("Failed to delete Assignment entity. Id: {Id}, Duration: {Duration}ms",
                    command.Id, stopwatch.ElapsedMilliseconds);
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Error processing DeleteAssignmentCommand. Id: {Id}, Duration: {Duration}ms",
                command.Id, stopwatch.ElapsedMilliseconds);
