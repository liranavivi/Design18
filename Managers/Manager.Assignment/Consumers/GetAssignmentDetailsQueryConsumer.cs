...
public class GetAssignmentDetailsQueryConsumer : IConsumer<GetAssignmentDetailsQuery>
{
    ...
    public async Task Consume(ConsumeContext<GetAssignmentDetailsQuery> context)
    {
        ...
        _logger.LogInformation("Processing GetAssignmentDetailsQuery. AssignmentId: {AssignmentId}, RequestedBy: {RequestedBy}",
        ...
        try
        {
            ...
            if (entity != null)
            {
                _logger.LogInformation("Successfully processed GetAssignmentDetailsQuery. Found Assignment Id: {Id}, StepId: {StepId}, EntityIds count: {EntityIdsCount}, Duration: {Duration}ms",
            ...
            {
                _logger.LogWarning("Assignment entity not found. AssignmentId: {AssignmentId}, Duration: {Duration}ms",
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Error processing GetAssignmentDetailsQuery. AssignmentId: {AssignmentId}, Duration: {Duration}ms",
                query.AssignmentId, stopwatch.ElapsedMilliseconds);
