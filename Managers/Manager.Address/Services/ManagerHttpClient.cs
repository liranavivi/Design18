...
public class ManagerHttpClient : IManagerHttpClient
{
    ...
    public ManagerHttpClient(HttpClient httpClient, IConfiguration configuration, ILogger<ManagerHttpClient> logger)
    {
        ...
        _assignmentManagerBaseUrl = configuration["ManagerUrls:Assignment"] ?? "http://localhost:60887";
        _schemaManagerBaseUrl = configuration["ManagerUrls:Schema"] ?? "http://localhost:60883";
    ...
    private async Task<bool> ExecuteEntityCheck(string url, string operationName, Guid entityId)
    {
        try
        {
            _logger.LogDebug("Starting {Operation} for EntityId: {EntityId}, URL: {Url}", 
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogDebug("Completed {Operation} for EntityId: {EntityId}. HasReferences: {HasReferences}", 
            ...
            {
                _logger.LogError("Failed {Operation} for EntityId: {EntityId}. StatusCode: {StatusCode}, URL: {Url}", 
        ...
        {
            _logger.LogError(ex, "Circuit breaker is open for {Operation}. EntityId: {EntityId}", 
        ...
        {
            _logger.LogError(ex, "Unexpected error during {Operation} for EntityId: {EntityId}", 
                operationName, entityId);
