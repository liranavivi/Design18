...
public class EntityReferenceValidator : IEntityReferenceValidator
{
    ...
    {
        _logger.LogDebug("Checking assignment references for EntityId: {EntityId}", entityId);

        try
        {
            var hasReferences = await _managerHttpClient.CheckEntityReferencesInAssignments(entityId);
            
            _logger.LogDebug("Assignment reference check completed for EntityId: {EntityId}. HasReferences: {HasReferences}", 
        ...
        {
            _logger.LogError(ex, "Assignment reference check failed for EntityId: {EntityId}", entityId);
    ...
    {
        _logger.LogDebug("Validating entity can be deleted. EntityId: {EntityId}", entityId);
        ...
        try
        {
            ...
            if (hasReferences)
            {
                ...
                _logger.LogWarning("Entity deletion blocked due to references. EntityId: {EntityId}, Message: {Message}", 
                    entityId, message);
                
                throw new InvalidOperationException(message);
            }
            
            _logger.LogDebug("Entity validation passed - no references found. EntityId: {EntityId}", entityId);
        }
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Entity validation failed due to service unavailability. EntityId: {EntityId}, Message: {Message}", 
                entityId, message);
    ...
    {
        _logger.LogDebug("Validating entity can be updated. EntityId: {EntityId}", entityId);

        // For now, we don't block updates based on references
        // This method is provided for future extensibility
        // If needed, similar validation logic can be implemented here
        
        _logger.LogDebug("Entity update validation passed. EntityId: {EntityId}", entityId);
