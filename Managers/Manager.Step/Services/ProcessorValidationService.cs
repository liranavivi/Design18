...
public class ProcessorValidationService : IProcessorValidationService
{
    ...
    public async Task ValidateProcessorExists(Guid processorId)
    {
        ...
        {
            _logger.LogDebug("Processor validation is disabled. Skipping validation for ProcessorId: {ProcessorId}", processorId);
            return;
        }
        ...
        if (processorId == Guid.Empty)
        {
            ...
            _logger.LogWarning("Processor validation failed: {Message}", message);
            throw new InvalidOperationException(message);
        }

        _logger.LogDebug("Validating processor exists. ProcessorId: {ProcessorId}", processorId);
        ...
        try
        {
            ...
            _logger.LogDebug("Processor validation passed. ProcessorId: {ProcessorId}", processorId);
        }
        ...
        {
            _logger.LogError(ex, "Processor validation failed due to unexpected error. ProcessorId: {ProcessorId}", processorId);
    ...
    public async Task ValidateNextStepsExist(List<Guid> nextStepIds)
    {
        if (!_enableProcessorValidation)
        {
            _logger.LogDebug("Next step validation is disabled. Skipping validation for NextStepIds: {NextStepIds}",
        ...
        {
            _logger.LogDebug("NextStepIds is null or empty. Skipping validation.");
            return;
        }
        ...
        if (emptyGuids.Any())
        {
            ...
            _logger.LogWarning("Next step validation failed: {Message}", message);
            throw new InvalidOperationException(message);
        }

        _logger.LogDebug("Validating next steps exist. NextStepIds: {NextStepIds}", string.Join(",", nextStepIds));
        ...
        var validationTasks = nextStepIds.Select(async stepId =>
        {
            ...
            {
                _logger.LogError(ex, "Failed to validate step existence. StepId: {StepId}", stepId);
        ...
        try
        {
            ...
            _logger.LogDebug("Next step validation passed. All NextStepIds exist: {NextStepIds}", string.Join(",", nextStepIds));
        }
        catch (InvalidOperationException)
        {
            // Re-throw validation exceptions as-is
        ...
        {
            _logger.LogError(ex, "Next step validation failed due to unexpected error. NextStepIds: {NextStepIds}",
                string.Join(",", nextStepIds));

            // Fail-safe approach: if validation service is unavailable, reject the operation
            throw new InvalidOperationException($"Next step validation service is unavailable. Operation rejected for data integrity.", ex);
        }
    }
}
