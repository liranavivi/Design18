...
public class DeleteStepCommandConsumer : IConsumer<DeleteStepCommand>
{
    ...
    public async Task Consume(ConsumeContext<DeleteStepCommand> context)
    {
        ...
        _logger.LogInformation("Processing DeleteStepCommand. Id: {Id}, RequestedBy: {RequestedBy}",
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Step entity not found for deletion. Id: {Id}", command.Id);
            ...
            if (success)
            {
                ...
                stopwatch.Stop();
                _logger.LogInformation("Successfully processed DeleteStepCommand. Id: {Id}, Duration: {Duration}ms",
            ...
            {
                stopwatch.Stop();
                _logger.LogWarning("Failed to delete Step entity. Id: {Id}, Duration: {Duration}ms",
                    command.Id, stopwatch.ElapsedMilliseconds);
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "Error processing DeleteStepCommand. Id: {Id}, Duration: {Duration}ms",
                command.Id, stopwatch.ElapsedMilliseconds);
